...
import { getRootTodos, getChildren } from "@/app/[locale]/(private)/_helpers/todos";
import { TodoBody } from "./todo-item";
import type { TodoDto } from "@/dto/todo";
import type { TodoStatus } from "../_validations/todo-status";
import { toast } from "sonner";

const COLUMNS: Array<{ status: TodoStatus; labelKey: string; dotClass: string }> = [
  { status: "pending", labelKey: "dashboard.pending", dotClass: "bg-amber-500" },
  { status: "in-progress", labelKey: "dashboard.inProgress", dotClass: "bg-blue-500" },
  { status: "done", labelKey: "dashboard.done", dotClass: "bg-emerald-500" },
];
...
export function KanbanBoard({ onEdit, onDelete, onAddSubtask }: KanbanBoardProps) {
  ...
  function handleDrop(todoId: number, newStatus: TodoStatus) {
    updateTodo.mutate(
      ...
      { onError: () => toast.error(t("kanban.statusUpdateFailed")) }
    );
  }

  return (
    <div className="grid grid-cols-1 gap-4 pb-20 sm:grid-cols-3">
      {COLUMNS.map(({ status, labelKey, dotClass }) => {
    ...
    >
      {depth === 0 && (
        <p className="mb-1.5 text-[0.6875rem] font-bold uppercase tracking-wider text-muted-foreground">
          {t("subtasksCount", { count: children.length })}
        </p>
      )}
      {children.map((c) => (
        <div key={c.id}>
